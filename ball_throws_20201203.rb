...
(1..MAX_FRAME).each do |frame|
  ...
  scores = [] # １フレームのスコア
  (1..3).each do |throws|
    remaining_pins = ball_throw.calc_pins(scores, score, frame)
    score = ball_throw.one_throw(remaining_pins)
    ball_throw.fill_in_score(scores, score)
  ...
  ball_throw.fill_in_score(total_scores, scores)
end
